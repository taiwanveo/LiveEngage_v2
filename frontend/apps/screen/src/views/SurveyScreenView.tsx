...
import { interactionTypeLabel, PRESENT_PAGE_TITLE_CLASS } from "@liveengage/ui";
...
export function SurveyScreenView({ surveyId, title }: Props): React.JSX.Element {
  const resultsQuery = useQuery({
    queryKey: ["survey-screen", surveyId],
        ...
        {resultsQuery.isLoading ? (
          ...
          <ul className="mx-auto max-w-5xl space-y-8">
            {questions.map((q, index) => (
              <SurveyQuestionBlock key={q.child_interaction_id} index={index + 1} question={q} />
            ))}
          </ul>
        )}
      </div>
...
function SurveyQuestionBlock({
  ...
  const maxBar = Math.max(1, ...ratingBars.map((r) => r.count), ...optionBars.map((o) => o.count));
