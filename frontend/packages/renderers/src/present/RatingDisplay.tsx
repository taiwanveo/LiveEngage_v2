    ...
    <div className="space-y-4">
      {average != null ? (
        <p className={large ? "text-5xl font-bold" : "text-3xl font-bold"}>
          {average.toFixed(1)}
          <span className={large ? "ml-2 text-2xl text-slate-300" : "ml-2 text-base text-muted"}>
            / {max}
          </span>
        </p>
      ) : null}
        ...
        {Array.from({ length: max - min + 1 }, (_, i) => min + i).map((value) => {
          ...
          return (
            <li key={value} className="flex items-center gap-3 text-sm">
              <span className="w-6 tabular-nums text-foreground">{value}</span>
              <div
                className={
                  large
                    ? "h-3 flex-1 overflow-hidden rounded-full bg-white/10"
                    : "h-2 flex-1 overflow-hidden rounded-full bg-surface-elevated"
                }
              >
                <div
