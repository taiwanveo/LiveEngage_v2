...
> **接手必讀**：Zeabur（或同類 PaaS）上本專案以 **monorepo** 部署為 **6 個獨立服務**（含 Screen 投影）。  
...
| 3 | **join** | 前端 | https://le-join.zeabur.app | [`frontend/Dockerfile.join`](../frontend/Dockerfile.join) |
| 3b | **participant**（舊網域轉址） | 前端 | https://le-participant.zeabur.app → join | [`frontend/Dockerfile.participant`](../frontend/Dockerfile.participant) |
| 3c | **screen** | 前端 | https://le-screen.zeabur.app | [`frontend/Dockerfile.screen`](../frontend/Dockerfile.screen) |
...
四個前端在建置時以 `VITE_API_BASE=https://le-api.zeabur.app` 指向 **api**；**host** 另設 `VITE_SCREEN_BASE` 供分享投影連結。
...
| 大螢幕投影 | **Screen App**（`le-screen`）由 Host 遙控；舊 `#/…/present` 路由過渡保留 |
...
**程式目錄**：[`frontend/apps/join`](../frontend/apps/join)

**生產網域**：`https://le-join.zeabur.app`  
**舊網域**：`https://le-participant.zeabur.app` 永久導向 join（保留 QR／書籤；`#/join/…` 以 HTML+JS 保留 hash）。
...
**使用者**：外接螢幕、LED Wall、OBS Browser Source（**screen token**，免 Host 登入）

**負責**：

| 功能 | 說明 |
|------|------|
| 遙控切換 | Host `PUT /rooms/{id}/screen` → WS `screen_view_changed` |
| 六種視圖 | standby / test / overview / poll / qa / quiz / ideas / survey |
| URL | `#/?event=CODE&token=…` 或 `#/?room=UUID&token=…`；可選 `theme` / `bg` / `fg` |
| 全螢幕 | 按 **F**；同機 Host 可 `postMessage` 遙控 |

**程式目錄**：[`frontend/apps/screen`](../frontend/apps/screen)  
**生產網域**：`https://le-screen.zeabur.app`
...
| join | `cd frontend/apps/join && npm run dev` |
